...
static unsigned int adrenoboost = 0; // disable
...
static const struct device_attribute *adreno_tz_attr_list[] = {
    &dev_attr_gpu_load, &dev_attr_suspend_time, &dev_attr_adrenoboost, NULL};
...
static int __secure_tz_reset_entry2(unsigned int *scm_data, u32 size_scm_data,
  ...
  if (!is_64) {
    ...
    spin_lock(&tz_lock);
    ret = scm_call2_atomic(SCM_SIP_FNID(SCM_SVC_IO, TZ_RESET_ID), &desc);
    spin_unlock(&tz_lock);
  } else {
...
static int __secure_tz_update_entry3(unsigned int *scm_data, u32 size_scm_data,
  ...
  if (!priv->is_64) {
    ...
    spin_lock(&tz_lock);
    ret = scm_call2_atomic(SCM_SIP_FNID(SCM_SVC_IO, TZ_UPDATE_ID), &desc);
    spin_unlock(&tz_lock);
    *val = ret;
  } else {
    unsigned int cmd_id;
...
static int tz_get_target_freq(struct devfreq *devfreq, unsigned long *freq) {
  ...
  *freq = stats.current_frequency;
  ...
  // scale busy time up based on adrenoboost parameter, only if MIN_BUSY
  // exceeded...
